#!/usr/bin/env bash
# 12_run_mlperf_accuracy.sh — dispatch MLPerf inference accuracy scenario.
#
# SCAFFOLD — same legitimacy-gate pattern as 11_run_mlperf_performance.sh.
#
# If verdict is STRICT_COMPLIANT:
#   Dispatches via POST /api/mp-exam/create with accuracy scenario parameters.
#
# If verdict is anything else:
#   Exits 78 (sysexits EX_CONFIG).
#
# Usage:
#   ./12_run_mlperf_accuracy.sh [--dry-run] [--help]
#
# Exit codes:
...
log "This script dispatches MLPerf ACCURACY scenario."
...
if [ ! -f "$LEGITIMACY_REPORT" ]; then
  ...
  log "  Run 15_validate_legitimacy.sh first."
  exit 78
fi
...
if [ "$VERDICT" != "STRICT_COMPLIANT" ]; then
  ...
  log "  Accuracy scenario dispatch is blocked."
  log "  Exiting with code 78 (EX_CONFIG)."
  exit 78
fi

log "  [OK]  Verdict is STRICT_COMPLIANT — proceeding with accuracy dispatch"

# ---------------------------------------------------------------------------
# 2. Load parameters from cluster.yaml
# ---------------------------------------------------------------------------
...
# Accuracy mode: smaller data_number, accuracy metric enabled
...
body = {
    ...
    'model': '$MODEL',
    'run_id': '$RUN_ID',
    'accuracy_metric': 'token_accuracy',
    'tags': ['mlperf-5.1', 'accuracy']
}
...
if [ "$DRY_RUN" = "true" ]; then
  ...
  log "=== 12_run_mlperf_accuracy: DRY-RUN COMPLETE ==="
  exit 0
fi

HTTP_CODE="$(curl -s -o "$LOG_DIR/12_dispatch_response.json" \
  -w '%{http_code}' \
  -X POST \
...
log "  Response saved to: $LOG_DIR/12_dispatch_response.json"

if [ "$HTTP_CODE" -ge 200 ] && [ "$HTTP_CODE" -lt 300 ] 2>/dev/null; then
  log "  [OK]  Accuracy scenario dispatched successfully"
  log "=== 12_run_mlperf_accuracy: COMPLETE ==="
  exit 0
else
  log "  [FAIL] Dispatch returned HTTP $HTTP_CODE"
  log "=== 12_run_mlperf_accuracy: FAILED ==="
  exit 1
fi
