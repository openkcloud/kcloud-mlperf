...
class MLPerfConfig:
    ...
    def __init__(self):
        ...
        self.username = os.environ.get('MLPERF_USERNAME', os.environ.get('USER', 'user'))
        self.remote_base_dir = os.environ.get('MLPERF_REMOTE_DIR', f'~/{Path(__file__).parent.name}')
        
        # Node configuration - can be overridden with environment variables
        self.nodes = {
            'jw1': os.environ.get('JW1_IP', 'localhost'),
            'jw2': os.environ.get('JW2_IP', 'localhost'), 
            'jw3': os.environ.get('JW3_IP', 'localhost')
        }
