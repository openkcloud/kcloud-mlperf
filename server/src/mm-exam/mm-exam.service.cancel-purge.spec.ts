...
// When a user stops an MM exam mid-run, partial result rows must be purged
// from mm_exam_result BEFORE the exam row is marked STOPPED. Otherwise a
...
describe('MmExamService.stop — cancel-and-purge (US-NEXT-5)', () => {
  ...
  beforeEach(async () => {
    ...
    mockExamRepo = {
      ...
      findOne: jest.fn().mockResolvedValue({ id: 7, status: 'Stopped' }),
    ...
    mockResultRepo = {
      ...
      find: jest.fn().mockResolvedValue([
        { id: 1, exam_id: 7, result_number: 1 },
        { id: 2, exam_id: 7, result_number: 2 },
      ]),
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MmExamService,
        { provide: getRepositoryToken(MmExam), useValue: mockExamRepo },
        {
  ...
  it('happy path: purges mm_exam_result rows by exam_id then marks exam STOPPED', async () => {
    await service.stop(7);

    expect(mockResultRepo.delete).toHaveBeenCalledTimes(1);
    expect(mockResultRepo.delete).toHaveBeenCalledWith({ exam_id: 7 });
